    ...
    if (req.body.repassword) {
        ...
        if (req.body.token) {
            ...
            if (req.body.token == getMD5Password(req.body.user_id)) {
                user.findOne({_id: req.body.user_id}, {password: req.body.password}, function (err, checkUser) {
                    if (checkUser) {
                        user.update({_id: req.body.user_id}, {password: req.body.password}, function (err, userUpdate) {
                            if (err) {
        ...
        user.findUserPassword(req.body.username, req.body.userMail, req.body.userPhone, function (err, userFound) {
            if (userFound.length != 0) {
                ...
                res.json({status: 1, messgae: "信息错误"})
            }
        })
    }
});
    ...
    if (req.body.token == getMD5Password(req.body.user_id)) {
        ...
        user.findByUsername(req.body.toUserName, function (err, toUser) {
            if (toUser.length != 0) {
                ...
                res.json({status:1,message:"发送的对象不存在"+req.body.token})
            }
        })
    } else {
        res.json({status: 1, messgae: "用户登陆错误"})
    }
});
//用户显示站内信，其中receive参数为1时是发送的内容，为2时是接收到的内容
router.post('/showEmail',function (req,res,next) {
        ...
        if(req.body.token == getMD5Password(req.body.user_id)){
            if(req.body.receive == 1){
                //发送站内信
                mail.findByFromUserd(req.body.user_id,function (err,sendMail) {
                            res.json({status:0,message:"获取成功",data:sendMail})
                })
            }else{
                //收到的站内信
                mail.findByToUserId(req.body.user_id,function(err,receivermail){
                    res.json({status:0,message:"获取成功",data:receivermail})
                })
            }
        }else{
            res.json({status:1,message:"用户登录错误"})
...
router.post('/download', function (req, res, next) {
    if (!req.body.movie_id) {
        res.json({status: 1, message: "电影id获取失败"})
    }
    movie.findById(req.body.movie_id, function (err, supportMovie) {
        //更新操作
